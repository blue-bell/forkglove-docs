...
| <b style={{whiteSpace: "nowrap"}}>Color by</b>    | Only shown if "Color mode" is not set to "Flat"; value used for "Color map" coloring logic; any numeric field in message such as `x`, `y`, `z`, `<distance>` (L2 norm of coordinates), or custom defined field                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| <b style={{whiteSpace: "nowrap"}}>Color map</b>   | Only shown if "Color mode" is set to "Color map"; "Turbo" (Google) or "Rainbow" (RViz); for mapping "Color by" field values to colors                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
Visualize [transform frames](#transforms) to debug why elements may not be rendering where expected.
...
| field            | description                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| ---------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Source**       | Source of the URDF<ul><li>**URL** – URL to the source URDF file in one of the following schemes:<ul><li>`http(s)://` – Over an HTTP(S) connection</li><li>`package://` – Package path. Only supported when using the desktop app or a live connection that supports asset fetching, such as [`foxglove_bridge`](/docs/2-connecting-to-data/4-ros-foxglove-bridge.md).</li></ul></li><li>**File path** – Absolute file path to the source URDF file. Only supported when using the desktop app.</li><li>**Parameteter** – Parameter containing the source URDF file.</li><li>**Topic** – Topic containing the source URDF file.</li></ul> |
...
1. When using the desktop app, package paths can be resolved by finding packages in directories specified by the [ROS_PACKAGE_PATH](/docs/visualization/introduction#general) setting
2. When using a live connection that supports asset fetching such as [Foxglove Bridge](/docs/2-connecting-to-data/4-ros-foxglove-bridge.md), assets can be fetched over the websocket connection
...
![3d panel clicked marker](/img/docs/visualization/panels/3d/clicked-marker.jpeg)

Click the icons next to the topic to open its settings in the sidebar or to display its messages in a new [Raw Messages panel](/docs/visualization/panels/raw-messages).
...
The desktop app uses Electron (based on Chromium), and should automatically use hardware acceleration when available. If you're seeing issues here, we recommend following the steps above to see if the same issue exists in Chrome. If so, there's a good chance it's affecting our application for the same reasons. Therefore we recommend getting things working in Chrome first, and if the desktop app is still having issues afterward please [let us know](https://foxglove.dev/slack) and we'll look into it.
...
If you are experiencing poor performance with the 3D panel while using `foxglove.CompressedVideo` topics, check that video acceleration is enabled. [Docs here](./image.md#checking-video-acceleration).
